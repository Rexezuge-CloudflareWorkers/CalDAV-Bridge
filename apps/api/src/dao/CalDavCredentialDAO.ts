...
class CalDavCredentialDAO {
  ...
  public async create(
    ...
    await this.database
      ...
      .bind(credentialId, applicationId, username, passwordHash, name, passwordPrefix, passwordLastFour, now, expiresAt)
      .run();
  ...
  public async getByUsernameAndHash(
    ...
    passwordHash: string,
    activeOnly: boolean,
  ): Promise<CalDavCredentialMetadata | undefined> {
    const now = TimestampUtil.getCurrentUnixTimestampInSeconds();
    ...
    const row = await this.database
      .prepare(
        `
          SELECT credential_id, application_id, username, password_hash, name, password_prefix, password_last_four, created_at, expires_at, last_used_at
  ...
  public async countByApplication(applicationId: string): Promise<number> {
    const row = await this.database
      .prepare('SELECT COUNT(*) AS count FROM caldav_credentials WHERE application_id = ?')
      .bind(applicationId)
      .first<{ count: number }>();
    return row?.count ?? 0;
  }
  ...
  public async usernameExists(username: string): Promise<boolean> {
    const row = await this.database
      .prepare('SELECT 1 AS found FROM caldav_credentials WHERE username = ? LIMIT 1')
      .bind(username)
      .first<{ found: number }>();
  ...
  public async deleteForApplication(credentialId: string, applicationId: string): Promise<void> {
    await this.database
      .prepare('DELETE FROM caldav_credentials WHERE credential_id = ? AND application_id = ?')
      .bind(credentialId, applicationId)
      .run();
  }
