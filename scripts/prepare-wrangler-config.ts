...
const DEFAULT_KV_NAMESPACE_NAMES: Record<string, string> = {
  OAUTH2_TOKEN_CACHE: 'caldav-bridge-oauth2-token-cache',
};
...
function getKVNamespaceName(config: WranglerConfig, binding: string): string {
  return DEFAULT_KV_NAMESPACE_NAMES[binding] ?? `${config.name ?? 'caldav-bridge'}-${binding.toLowerCase()}`;
}
...
function ensureKVNamespace(config: WranglerConfig, binding: string): string {
  ...
  const candidateNames = new Set([namespaceName, `${config.name ?? 'caldav-bridge'}-${binding}`, binding]);
